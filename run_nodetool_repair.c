...
int run_nodetool_repair() {
	fprintf(stderr, "Launching nodetool repair...\n\n\n");
	int ret = system("/usr/bin/nodetool repair -pr");
...
void lock_or_watch(){
	...
	RET1 = zoo_create( zh, zoo_path, thishost, strlen(thishost), &ZOO_OPEN_ACL_UNSAFE, ZOO_EPHEMERAL, NULL, 0);
        if (RET1 == ZOK)
        {
		fprintf(stderr, "Lock set. Safe for this host to proceed running nodetool repair.\n");
		RET2 = run_nodetool_repair();
		fprintf(stderr, "Nodetool repair has finished with return code: %d\n", RET2);
